...
// 5) Rollup config compatibility: enforce @rollup/plugin-typescript usage
function replaceTs2InFile(fp) {
  if (!exists(fp)) return false;
  const src = fs.readFileSync(fp, 'utf8');
  let out = src;
  out = out.replace(/import\s+[^\s]+\s+from\s+['\"]rollup-plugin-typescript2['\"];?/g, "import typescript from '@rollup/plugin-typescript';");
  out = out.replace(/const\s+[^\s]+\s*=\s*require\(['\"]rollup-plugin-typescript2['\"]\);?/g, "const typescript = require('@rollup/plugin-typescript');");
  // Fallback: direct string swap in case config uses dynamic patterns
  out = out.replace(/['\"]rollup-plugin-typescript2['\"]/g, "'@rollup/plugin-typescript'");
  if (out !== src) {
...
function findRollupConfigs(dir) {
  const candidates = [];
  const names = ['rollup.config.ts', 'rollup.config.mts', 'rollup.config.js', 'rollup.config.cjs'];
...
const pkgDirs = [
  path.join(lcDir, 'packages', 'data-provider'),
  path.join(lcDir, 'packages', 'data-schemas'),
];

for (const d of pkgDirs) {
  for (const fp of findRollupConfigs(d)) {
    replaceTs2InFile(fp);
  }
}

// 6) Safety shim: ensure require('rollup-plugin-typescript2') resolves to @rollup/plugin-typescript
function ensureShim(pkgDir) {
  const shimDir = path.join(pkgDir, 'node_modules', 'rollup-plugin-typescript2');
  const indexJs = path.join(shimDir, 'index.js');
  const pkgJson = path.join(shimDir, 'package.json');
  try {
    fs.mkdirSync(shimDir, { recursive: true });
    if (!exists(indexJs)) {
      fs.writeFileSync(indexJs, "module.exports = require('@rollup/plugin-typescript');\n", 'utf8');
    }
    if (!exists(pkgJson)) {
      fs.writeFileSync(pkgJson, JSON.stringify({ name: 'rollup-plugin-typescript2', version: '0.0.0-shim' }, null, 2));
    }
    console.log('[voygent patch] Added shim for rollup-plugin-typescript2 in', pkgDir);
  } catch (e) {
    console.log('[voygent patch] Failed to create shim in', pkgDir, e?.message || e);
  }
}

for (const d of pkgDirs) ensureShim(d);
